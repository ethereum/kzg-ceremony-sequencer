...
fn oauth_client() -> BasicClient {
    ...
    let auth_url = env::var("AUTH_URL")
        .unwrap_or_else(|_| "https://kev-kzg-ceremony.eu.auth0.com/authorize".to_string());

    let token_url = env::var("TOKEN_URL")
        .unwrap_or_else(|_| "https://kev-kzg-ceremony.eu.auth0.com/oauth/token".to_string());
...
// Session is optional
async fn index(user: Option<Auth0User>) -> impl IntoResponse {
    match user {
        Some(u) => format!(
...
async fn protected(user: Auth0User) -> impl IntoResponse {
    format!(
...
#[async_trait]
impl<B> FromRequest<B> for Auth0User
where
    B: Send,
{
    ...
    async fn from_request(req: &mut RequestParts<B>) -> Result<Self, Self::Rejection> {
        ...
        let user = session.get::<Auth0User>("user").ok_or(AuthRedirect)?;
